...
// import utils
import { readValidator, withdraw } from './utils.ts';
...
lucid.selectWalletFromPrivateKey(await Deno.readTextFile('./keys/otherKey.sk'));

// set up validator
const validator = readValidator();

// withdraw fail test case
Deno.test('Withdraw Fail', async () => {
  // prepare contract, fake withdrawer
  console.log(colors.bold(colors.cyan('Prepare Contract, Fake Withdrawer')));
  const contractAddress = lucid.utils.validatorToAddress(validator);
  console.log(`
    ${colors.magenta('Contract Address is')}: ${colors.bold(
    colors.brightGreen(contractAddress)
  )}\n
    ${colors.magenta('Fake Withdrawer Address is')}: ${colors.bold(
    colors.brightRed(await lucid.wallet.address())
  )}
`);
